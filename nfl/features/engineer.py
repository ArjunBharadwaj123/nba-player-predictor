...
def feature_columns(df: pd.DataFrame) -> list[str]:
    ...
    suffix) OR an explicit context/opponent/missing-indicator column. Raw
    same-week stat columns are never features.
    """
    allow = set(_CONTEXT_ALLOW) | set(_OPPONENT_ALLOW) | set(MISSING_INDICATORS)
    feats: list[str] = []
    for c in df.columns:
